...
import { exportIngredients, exportRecipes, exportOrders, exportClients } from "../../utils/export.js";
...
export const Dashboard = () => {
    ...
    const createNewIngredientForRecipeStay = async () => {
        ...
        try {
            const r = await apiFetch(`${backendUrl}/api/ingredients`, {
                method: 'POST',
        ...
        <>
            ...
            <div className={`dashboard ${!darkMode ? 'light-mode' : ''}`}>
                ...
                <div className="mobile-header">
                    ...
                    <h1><i className="fas fa-utensils text-warning me-2"></i>ChefSync</h1>
                ...
                {showModal && selectedRecipe && (
                    <RecipeViewModal
                        ...
                        editMode={editConfirm}
                        setEditMode={setEditConfirm}
                        editRecipe={editRecipe}
                        setEditRecipe={setEditRecipe}
                        editIngredients={editIngredients}
                        setEditIngredients={setEditIngredients}
                        selectedIngToEdit={selectedIngToEdit}
                        setSelectedIngToEdit={setSelectedIngToEdit}
                        store={store}
                        onClose={() => { setShowModal(false); setEditConfirm(false); }}
                        onEditIngredient={toggleIngredientEdit}
                        onRemoveChecked={removeCheckedIngredients}
                        onAddIngredient={() => { if (selectedIngToEdit === 'new') setShowNewIngredientEdit(true); else addIngredientToEdit(); }}
                ...
                {showNewRecipe && (
                    <NewRecipeModal
                        show={showNewRecipe}
                        onClose={() => { setShowNewRecipe(false); setNewRecipeIngredients([]); setSelectedIngToAdd(''); }}
                        newRecipe={newRecipe}
                        ...
                        onAddIngredient={() => { if (selectedIngToAdd === 'new') { setIngredientSource('recipe'); setShowNewIngredient(true); } else { addIngredientToRecipe(); } }}
